...
- **Cross-roadmap autopilot mode (`--all-roadmaps`)**: discovers every
  **open** roadmap root (an open issue carrying the `roadmap` label **or**
  an `<!-- kurone-kito-roadmap-id: ... -->` marker **or** a
  ...
  - **Opt-in leaf annotations** (additive; absent flags leave the leaf shape
    ...
    and hidden `kurone-kito-blocked-by` markers, plus
...
- **Legacy roots (`discover.legacyRoots`, #1315)**: a repository that
  ...
  label and the `kurone-kito-roadmap-id` marker, so they
...
`idd-review-fix.instructions.md`'s E11 both merge `master` into the feature
branch with `git fetch origin master && git merge origin/master`. On a repo
