...
2. If the target issue carries the configured roadmap label or an
   `kurone-kito-roadmap-id` marker — the same test
   **A2**'s roadmap-node/execution-leaf classification rule uses (an
   ...
   - Apply **A3**'s dependency bullet to the target itself (both
     ...
     `kurone-kito-blocked-by` markers) and its
...
NOT contain an `kurone-kito-roadmap-id` marker (not itself
a roadmap) or an `kurone-kito-blocked-by` marker, AND
...
`kurone-kito-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`kurone-kito-roadmap-id` marker. Retro-label a legacy
...
`<!-- kurone-kito-roadmap-id: ... -->` marker are
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `kurone-kito-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`, or when
  ...
  `kurone-kito-roadmap-id` or
  `kurone-kito-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `kurone-kito-roadmap-id` marker when checking
  `kurone-kito-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- kurone-kito-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- kurone-kito-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- kurone-kito-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- kurone-kito-effort: S|M|L -->` footer
...
- **Roadmap identity** (`kurone-kito-roadmap-id`): in the
...
- **Sequential dependency** (`kurone-kito-blocked-by`): in an
  issue body — this issue **cannot start until** the roadmap with the
  matching `roadmap-id` is closed.

**Do not use `kurone-kito-blocked-by` to group sub-tasks under
